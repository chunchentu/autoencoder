...
class ImageNet10:
  def __init__(self, selectClassSet, testRatio):
    np.random.seed(9487)
    ...
    for i in range(file_len):
      temp = readimg(file_list[i])
      if temp is None:
        continue
      else:
        temp_data, temp_label = temp
      if temp_label in selectClassSet:
        data_list.append(temp_data)
        labels_list.append(temp_label)
    testNum = np.floor(file_len*testRatio)
    trainNum = file_len - testNum

    class_map = {}
    cnt = -1
    for s in selectClassSet:
      cnt += 1
      class_map[s] = cnt
    print(class_map)
    classNum = cnt

    print("Select class {}: total data size {}, total selected class {}".format(classSet, file_len, classNum))
    print("Train size:{}, test size:{}".format(trainNum, testNum))
    self.train_data = np.array(data_list[:trainNum])
    self.train_labels = np.zeros((trainNum, classNum))
    self.train_labels[np.arange(trainNum), labels_list[:trainNum]] = 1

    self.test_data = np.array(data_list[trainNum:])
    self.test_labels = np.zeros((testNum, classNum))
    self.test_labels[np.arange(testNum), labels_list[trainNum:]] = 1
